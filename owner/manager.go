...
func NewSession(ctx context.Context, logPrefix string, etcdCli *clientv3.Client, retryCnt, ttl int) (*concurrency.Session, error) {
	...
	for i := 0; i < retryCnt; i++ {
		...
		if vcloseClient, __fpErr := __fp_closeClient.Acquire(); __fpErr == nil {
			defer __fp_closeClient.Release()
			closeClient, __fpTypeOK := vcloseClient.(bool)
			if !__fpTypeOK {
				goto __badTypecloseClient
			}
			if closeClient {
				etcdCli.Close()
			}
		__badTypecloseClient:
			__fp_closeClient.BadType(vcloseClient, "bool")
		}

		if vcloseGrpc, __fpErr := __fp_closeGrpc.Acquire(); __fpErr == nil {
			defer __fp_closeGrpc.Release()
			closeGrpc, __fpTypeOK := vcloseGrpc.(bool)
			if !__fpTypeOK {
				goto __badTypecloseGrpc
			}
			if closeGrpc {
				etcdCli.ActiveConnection().Close()
			}
		__badTypecloseGrpc:
			__fp_closeGrpc.BadType(vcloseGrpc, "bool")
		}
