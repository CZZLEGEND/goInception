...
func (s *session) StmtCommit() {
	...
	err := kv.WalkMemBuffer(st.buf, func(k kv.Key, v []byte) error {
		if vmockStmtCommitError, __fpErr := __fp_mockStmtCommitError.Acquire(); __fpErr == nil {
			defer __fp_mockStmtCommitError.Release()
			mockStmtCommitError, __fpTypeOK := vmockStmtCommitError.(bool)
			if !__fpTypeOK {
				goto __badTypemockStmtCommitError
			}
			if mockStmtCommitError {
				return errors.New("mock stmt commit error")
			}
		__badTypemockStmtCommitError:
			__fp_mockStmtCommitError.BadType(vmockStmtCommitError, "bool")
		}
		if len(v) == 0 {
