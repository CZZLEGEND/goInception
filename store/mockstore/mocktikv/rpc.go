...
func (c *RPCClient) SendRequest(ctx context.Context, addr string, req *tikvrpc.Request, timeout time.Duration) (*tikvrpc.Response, error) {
	if vrpcServerBusy, __fpErr := __fp_rpcServerBusy.Acquire(); __fpErr == nil {
		defer __fp_rpcServerBusy.Release()
		rpcServerBusy, __fpTypeOK := vrpcServerBusy.(bool)
		if !__fpTypeOK {
			goto __badTyperpcServerBusy
		}
		if rpcServerBusy {
			return tikvrpc.GenRegionErrorResp(req, &errorpb.Error{ServerIsBusy: &errorpb.ServerIsBusy{}})
		}
	__badTyperpcServerBusy:
		__fp_rpcServerBusy.BadType(vrpcServerBusy, "bool")
	}
	...
	case tikvrpc.CmdCommit:
		if vrpcCommitResult, __fpErr := __fp_rpcCommitResult.Acquire(); __fpErr == nil {
			defer __fp_rpcCommitResult.Release()
			rpcCommitResult, __fpTypeOK := vrpcCommitResult.(string)
			if !__fpTypeOK {
				goto __badTyperpcCommitResult
			}
		...
		__badTyperpcCommitResult:
			__fp_rpcCommitResult.BadType(vrpcCommitResult, "string")
		}
		r := req.Commit
		if err := handler.checkRequest(reqCtx, r.Size()); err != nil {
		...
		if vrpcCommitTimeout, __fpErr := __fp_rpcCommitTimeout.Acquire(); __fpErr == nil {
			defer __fp_rpcCommitTimeout.Release()
			rpcCommitTimeout, __fpTypeOK := vrpcCommitTimeout.(bool)
			if !__fpTypeOK {
				goto __badTyperpcCommitTimeout
			}
			if rpcCommitTimeout {
				return nil, undeterminedErr
			}
		__badTyperpcCommitTimeout:
			__fp_rpcCommitTimeout.BadType(vrpcCommitTimeout, "bool")
		}
