...
func (s *RegionRequestSender) SendReq(bo *Backoffer, req *tikvrpc.Request, regionID RegionVerID, timeout time.Duration) (*tikvrpc.Response, error) {
	...
	if vtikvStoreSendReqResult, __fpErr := __fp_tikvStoreSendReqResult.Acquire(); __fpErr == nil {
		defer __fp_tikvStoreSendReqResult.Release()
		tikvStoreSendReqResult, __fpTypeOK := vtikvStoreSendReqResult.(string)
		if !__fpTypeOK {
			goto __badTypetikvStoreSendReqResult
		}
	...
	__badTypetikvStoreSendReqResult:
		__fp_tikvStoreSendReqResult.BadType(vtikvStoreSendReqResult, "string")
	}
