...
func (w *addIndexWorker) run(d *ddlCtx) {
	...
	for {
		...
		if vmockAddIndexErr, __fpErr := __fp_mockAddIndexErr.Acquire(); __fpErr == nil {
			defer __fp_mockAddIndexErr.Release()
			mockAddIndexErr, __fpTypeOK := vmockAddIndexErr.(bool)
			if !__fpTypeOK {
				goto __badTypemockAddIndexErr
			}
		...
		__badTypemockAddIndexErr:
			__fp_mockAddIndexErr.BadType(vmockAddIndexErr, "bool")
		}
