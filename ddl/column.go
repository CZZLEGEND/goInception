...
func onAddColumn(d *ddlCtx, t *meta.Meta, job *model.Job) (ver int64, _ error) {
	...
	if verrorBeforeDecodeArgs, __fpErr := __fp_errorBeforeDecodeArgs.Acquire(); __fpErr == nil {
		defer __fp_errorBeforeDecodeArgs.Release()
		errorBeforeDecodeArgs, __fpTypeOK := verrorBeforeDecodeArgs.(bool)
		if !__fpTypeOK {
			goto __badTypeerrorBeforeDecodeArgs
		}
		if errorBeforeDecodeArgs {
			return ver, errors.New("occur an error before decode args")
		}
	__badTypeerrorBeforeDecodeArgs:
		__fp_errorBeforeDecodeArgs.BadType(verrorBeforeDecodeArgs, "bool")
	}
...
func doModifyColumn(t *meta.Meta, job *model.Job, newCol *model.ColumnInfo, oldName *model.CIStr, pos *ast.ColumnPosition) (ver int64, _ error) {
	...
	if vuninitializedOffsetAndState, __fpErr := __fp_uninitializedOffsetAndState.Acquire(); __fpErr == nil {
		defer __fp_uninitializedOffsetAndState.Release()
		uninitializedOffsetAndState, __fpTypeOK := vuninitializedOffsetAndState.(bool)
		if !__fpTypeOK {
			goto __badTypeuninitializedOffsetAndState
		}
	...
	__badTypeuninitializedOffsetAndState:
		__fp_uninitializedOffsetAndState.BadType(vuninitializedOffsetAndState, "bool")
	}
