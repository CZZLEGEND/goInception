...
func getReorgInfo(d *ddlCtx, t *meta.Meta, job *model.Job, tbl table.Table) (*reorgInfo, error) {
	...
	if job.SnapshotVer == 0 {
		...
		if verrorUpdateReorgHandle, __fpErr := __fp_errorUpdateReorgHandle.Acquire(); __fpErr == nil {
			defer __fp_errorUpdateReorgHandle.Release()
			errorUpdateReorgHandle, __fpTypeOK := verrorUpdateReorgHandle.(bool)
			if !__fpTypeOK {
				goto __badTypeerrorUpdateReorgHandle
			}
		...
		__badTypeerrorUpdateReorgHandle:
			__fp_errorUpdateReorgHandle.BadType(verrorUpdateReorgHandle, "bool")
		}
